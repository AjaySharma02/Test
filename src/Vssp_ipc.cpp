...
static VSSP_IPC_RET_T _Vssp_q_create(int* p_msg_handle,int key_value)
{
    ...
    key = ftok(FILENAME, key_value);
	if (key == -1)
	{
		REST_IPC_ERROR("ftok failed!\n");
		*p_msg_handle = 0;
		perror("ftok");
...
static VSSP_IPC_RET_T _Vssp_q_send_msg(int handle, REST_MSG_BUF* pv_msg)
{
    ...
    while(ret_val == -1 ) {
        ret_val = msgsnd(handle, pv_msg, REST_BUFFER_SIZE, IPC_NOWAIT);
        if (ret_val == -1) {
            perror("#### SND error");
            if (errno != EAGAIN) {
                break;
...
static VSSP_IPC_RET_T _Vssp_q_receive_msg(int handle,REST_MSG_BUF* pv_msg)
{
    ...
    while(ret_val == -1 ) {
        ret_val = msgrcv(handle, (void*)pv_msg, REST_BUFFER_SIZE, 0, 0);
        if (ret_val == -1) {
            perror("#### RCV error");
            if (errno != EAGAIN) {
                break;
...
VSSP_IPC_RET_T Vssp_ipc_init(void)
{
    ...
    ret_val = _Vssp_q_create(&(g_Vssp_ipc_msgq_t.Event_Msgq_Id),REST_EVENT_IPC_KEY);
    if(ret_val != VSSP_IPC_R_OK)
    {
        REST_IPC_ERROR("\ncreate app 2 msg queue failed!\n");
        return VSSP_IPC_R_FAIL;
    }

    REST_IPC_DEBUG("\ncreate msg queue finished!\n");
...
{
    _Vssp_q_destroy(g_Vssp_ipc_msgq_t.Event_Msgq_Id);
...
VSSP_IPC_RET_T Vssp_get_msg_from_rest_event(REST_MSG_BUF* pRecvMsg)
{
    ...
    VSSP_IPC_RET_T ret_val;
    ret_val = _Vssp_q_receive_msg(g_Vssp_ipc_msgq_t.Event_Msgq_Id,pRecvMsg);
    if(ret_val != VSSP_IPC_R_OK)
    {
        REST_IPC_ERROR("\n%s,%d,Vssp get notify message from REST failed!\n", __FUNCTION__, __LINE__);
        return VSSP_IPC_R_FAIL;
    }
